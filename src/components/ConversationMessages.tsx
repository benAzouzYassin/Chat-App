...
export default function ConversationMessages(props: Props) {
    const loggedUserId = props.loggedUser ? props.loggedUser.userId : null
    ...
    return <div className="h-[84vh] flex flex-col overflow-y-scroll gap-3 pt-3 pl-2 pr-8">
        ...
        {props.selectedUser && loggedUserId && props.messages.map(msg => <p className="text-md w-fit p-2 rounded-full " key={uuid4()} style={props.loggedUser?.userId === msg.senderId ? userMessageStyle : otherMessageStyle}>{msg.msgText}</p>)}
    </div >
