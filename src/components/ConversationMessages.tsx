...
export default function ConversationMessages(props: Props) {
    ...
    return <div className="h-[84vh] flex flex-col overflow-y-scroll gap-3 pt-3 pl-2 pr-8">
        {!selectedUser && <p className="text-6xl text-gray-400 font-medium italic  h-2/3 flex items-center justify-center ">No messages To show</p>}
        {selectedUser && loggedUser.userId && props.messages.map(msg => <p className="text-md w-fit p-2 rounded-full " key={uuid4()} style={loggedUser?.userId === msg.senderId ? userMessageStyle : otherMessageStyle}>{msg.text} (from {msg.senderId})</p>)}
    </div >
