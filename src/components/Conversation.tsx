...
export default function Conversation(props: Props) {
    const { addNewUser, conversations, } = useContext(ConversationsContext)
    ...
    return <div className=" w-full">
        ...
        <ConversationMessages selectedUser={props.selectedUser} key={props.selectedUser?.userId} messages={currentConvMessages} />
        {
            props.selectedUser &&
            <div className="w-full  h-[57px] flex flex-row overflow-x-hidden pl-5 gap-1" >
                ...
                {messageInput ? <SendIcon className="scale-150 mt-[6px] hover:cursor-pointer hover:scale-125 rounded-xl " onClick={() => sendMessage({ messageId: crypto.randomUUID(), senderName: loggedUser?.userName ?? "userName err", receiver: props.selectedUser?.userId ?? "receiver err", text: messageInput, senderId: loggedUser?.userId ?? "userId err", })} /> : <LikeIcon className="scale-110 mt-[6px] hover:cursor-pointer hover:scale-125 " />}
            </div>
        }
