...
export default function Chat() {
    ...
    const handleSendBtn = () => {
        const msg: Message = { userId: user?.id ?? "unknown", text: msgInput, messageId: uuidv4(), senderName: user?.userName ?? "unknown" }
        setMsgInput("")
    ...
    const handleJoinRoom = () => {
        socket.emit("join-room", { roomId: roomInput, userId: user?.id, userName: user?.userName })
    }
    ...
    useEffect(() => {
        ...
        socket.on("connect", () => socket.emit("join-room", user))
    ...
    return <div className="h-[100vh] px-36 pt-10 ">
        ...
        <div className="flex flex-row w-[80vw]">
            <input type="text" placeholder="Enter room id" className=" w-[85%] text-xl px-2 border-[2px] h-14 mt-2 rounded-md" value={roomInput} onChange={e => setRoomInput(e.target.value)} />
            <button className=" ml-2 hover:bg-blue-400 mt-2 bg-blue-300 rounded-md w-[15%] font-semibold text-lg" onClick={handleJoinRoom}>Join room</button>
        </div>
        <div className="flex flex-row w-[80vw]">
